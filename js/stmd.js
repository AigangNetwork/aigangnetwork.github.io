...
;(function(exports) {
        ...
        while (true) {
          ...
          if (res.numdelims >= 1 && res.numdelims <= 3 && res.can_close && res.numdelims != first_close_delims) {
            ...
            if (first_close > 0) {
              ...
              inlines.push({
                t: 'Str',
                c: this.subject.slice(this.pos - res.numdelims, this.pos)
              })
  ...
  var parseLink = function(inlines) {
    ...
    if (this.peek() == '(') {
      ...
      if (
        ...
        ((/^\s/.test(this.subject[this.pos - 1]) && (title = this.parseLinkTitle() || '')) || true) &&
        this.spnl() &&
        this.match(/^\)/)
      ) {
        inlines.push({
  ...
  var parseImage = function(inlines) {
    if (this.match(/^!/)) {
      ...
      if (n === 0) {
        inlines.push({ t: 'Str', c: '!' })
        return 1
      } else if (inlines[inlines.length - 1] && inlines[inlines.length - 1].t == 'Link') {
  ...
  var acceptsLines = function(block_type) {
    return block_type == 'Paragraph' || block_type == 'IndentedCode' || block_type == 'FencedCode'
  }
  ...
  var incorporateLine = function(ln, line_number) {
    ...
    while (
      ...
      if (indent >= CODE_INDENT) {
        ...
        // remove trailing ###s:
        container.strings = [ln.slice(offset).replace(/(?:(\\#) *#*| *#+) *$/, '$1')]
        break
    ...
    if (this.tip !== last_matched_container && !blank && this.tip.t == 'Paragraph' && this.tip.strings.length > 0) {
      ...
      switch (container.t) {
        ...
        default:
          if (acceptsLines(container.t)) {
            ...
            console.log(
              'Line ' + line_number.toString() + ' with container type ' + container.t + ' did not match any condition.'
            )
          }
      }
    }
  ...
  var finalize = function(block, line_number) {
    ...
    switch (block.t) {
      case 'Paragraph':
        ...
        while (
          block.string_content[0] === '[' &&
          (pos = this.inlineParser.parseReference(block.string_content, this.refmap))
        ) {
  ...
  var processInlines = function(block) {
    switch (block.t) {
      ...
      case 'SetextHeader':
      case 'ATXHeader':
        block.inline_content = this.inlineParser.parse(block.string_content.trim(), this.refmap)
        block.string_content = ''
        break
      default:
        break
  ...
  var renderInline = function(inline) {
    ...
    switch (inline.t) {
      ...
      case 'Image':
        attrs = [['src', this.escape(inline.destination, true)], ['alt', this.escape(this.renderInlines(inline.label))]]
        if (inline.title) {
          attrs.push(['title', this.escape(inline.title, true)])
        }
    ...
    switch (block.t) {
      ...
      case 'BlockQuote':
        ...
        return inTags(
          'blockquote',
          [],
          filling === '' ? this.innersep : this.innersep + this.renderBlocks(block.children) + this.innersep
        )
      case 'ListItem':
        return inTags('li', [], this.renderBlocks(block.children, in_tight_list).trim())
      case 'List':
        tag = block.list_data.type == 'Bullet' ? 'ul' : 'ol'
        attr = !block.list_data.start || block.list_data.start == 1 ? [] : [['start', block.list_data.start.toString()]]
        return inTags(tag, attr, this.innersep + this.renderBlocks(block.children, block.tight) + this.innersep)
      case 'ATXHeader':
      case 'SetextHeader':
        tag = 'h' + block.level
        return inTags(tag, [], this.renderInlines(block.inline_content))
      case 'IndentedCode':
        return inTags('pre', [], inTags('code', [], this.escape(block.string_content)))
      case 'FencedCode':
        ...
        return inTags('pre', [], inTags('code', attr, this.escape(block.string_content)))
      case 'HtmlBlock':
    ...
    return {
      ...
      escape: function(s, preserve_entities) {
        if (preserve_entities) {
          return s
            .replace(/[&](?![#](x[a-f0-9]{1,8}|[0-9]{1,8});|[a-z][a-z0-9]{1,31};)/gi, '&amp;')
            .replace(/[<]/g, '&lt;')
            .replace(/[>]/g, '&gt;')
            .replace(/["]/g, '&quot;')
        } else {
